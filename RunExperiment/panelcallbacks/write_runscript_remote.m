...
if saveWaves % save the stimulus timings;
    ...
    fprintf(fid,'filename=''stims.mat'';');
    fprintf(fid,'if exist(filename,''file'');filename=[''stims_ '' char(datetime(''now'',''format'',''hhmmss''))];end;');
    fprintf(fid,'save(filename,''-v7'',''MTI2'',''start'',''saveScript'',''StimWindowRefresh'',''NewStimViewingDistance'',''NewStimPixelsPerCm'');\n');
    fprintf(fid,'disp([''saved stimulus in '' filename]);cd(gggg);\n');
