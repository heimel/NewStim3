...
if prod(double((size(LBp.points)==[1 2])&(LBp.points==[0 0])))
    XY=zeros(width,height);
else 
    XY=drawpoly(LBp,pol); 
end
if prod(double((size(LBp.remove)==[1 2])&(LBp.remove==[0 0])))
    XYr=zeros(width,height);
else
    XYr=drawpoly(LBp,polr); 
end
inds=find(XY);
XY(XY==0)=1; 
XY(inds)=255; 
XY(find(XYr))=0; 
filt=ones(fix(LBp.smooth));
XY = conv2(XY,filt,'same');
XY=round(255*XY/max(max(XY)));
